...
% Case 3: Laplace velocity disturbance
%
% Nominal training dynamics:
%   p_dot = v
%   v_dot = a_t
%
% Case 3 evaluation dynamics:
%   p_{t+1} = p_t + dt*v_t
%   v_{t+1} = v_t + dt*a_t + epsilon_t
%   epsilon_t ~ Laplace(0, b)
%
% where:
%   b = sigma / sqrt(2)
%
% This keeps the Laplace disturbance variance comparable to Gaussian:
%   Var(Laplace(0,b)) = 2b^2 = sigma^2
%
% This tests robustness under heavy-tailed stochastic disturbance.
...
% Case 3: Laplace noise levels
% sigma = 0 is the deterministic condition
% sigma = 0.01 will be used later as the noisy baseline for alert threshold
...
for l = 1:numLambdas
    ...
    for s = 1:numSeeds
        ...
        %% Evaluate under Case 3 Laplace velocity disturbance
        ...
        for sigIdx = 1:numSigmas
            ...
            b = sigmaLevel / sqrt(2);
            u = rand(size(Xtest,1), 1) - 0.5;

            % Inverse-CDF sampling for Laplace(0,b)
            eps_v = -b .* sign(u) .* log(1 - 2*abs(u));
            ...
            %% Evaluation metrics under Case 3 Laplace disturbance
            ...
            % The Laplace disturbance is treated as stochastic deviation,
...
save("case3_laplace_results.mat", ...
    ...
    "resultsTable", "aucTable", "predictionError_cell");

writetable(resultsTable, "case3_laplace_results.csv");
writetable(aucTable, "case3_laplace_auc_summary.csv");
...
title("Case 3 Laplace disturbance: Next-state MSE");
...
title("Case 3 Laplace disturbance: ODE residual MSE");
legend("\sigma=0", "\sigma=0.01", "\sigma=0.05", "\sigma=0.1", "\sigma=0.2", ...
    "Location", "best");
grid on;
hold off;
...
title("Case 3: Nominal residual discrepancy for alert layer");
...
xlabel("\sigma");
ylabel("\lambda_{ODE}");
title("Case 3 heatmap: Next-state MSE");
...
xlabel("\sigma");
ylabel("\lambda_{ODE}");
title("Case 3 heatmap: Nominal discrepancy MSE");

fprintf("\nCase 3 Laplace disturbance experiment completed.\n");
fprintf("Saved: case3_laplace_results.mat\n");
fprintf("Saved: case3_laplace_results.csv\n");
fprintf("Saved: case3_laplace_auc_summary.csv\n");
